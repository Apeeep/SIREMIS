<?php
	...
	if(!empty($_POST['reg_no'])){
		...
		$kembalian 		= $total - $jumlah_bayar;
 		if($kasir->createbayar($reg_no,$total,$jumlah_bayar,$kembalian)){
			$sg   = "ok";
...
<form id="forms" method="post" onSubmit="return submitForm('<?php echo $_SERVER['PHP_SELF']; ?>')" class="form-horizontal" autocomplete="off">
	<fieldset>
		<div class="modal-header">
        	<h4 class="blue bigger">Tambah Pembayaran</h4>
     	 </div>
		<span>
     <?php
    ...
    ?>
		...
		<div class="modal-footer">
	      <?php if(!isset($_POST['harga'])) { echo " <button type='submit' class='btn btn-primary' ></i>Bayar</button>"; }?>
	      <?php echo "
	        <a href='javascript:void(0)' onclick='bayar($id)' class='btn btn-primary' ><i class='icon-close icon-white'></i>Tutup</a> "; ?>
    	</div>		
	</fieldset>
</form>
</div>
