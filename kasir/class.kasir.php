...
class kasir
{
	...
	public function createbayar($reg_no,$total,$jumlah_bayar,$kembalian)
	{
		try
		{
			$query = $this->db->prepare("INSERT INTO bayar(reg_no,total,jumlah_bayar,kembalian) VALUES (:1, :2, :3, :4)");
	...
	{
		$query = "SELECT kembalian FROM bayar WHERE reg_no = $id ";
		$query = $this->db->prepare($query);
		$query->execute();
		if($query->rowCount()>0){
			return true;
	...
	public function viewkasir($query)
	{
		...
		if($query->rowCount()>0)
		{
			while($row=$query->fetch(PDO::FETCH_ASSOC))
			{
                ...
                <tr>
	                <td><div align="center">
	                <?php echo "<a href='javascript:void(0)' onclick=\"bayar('$row[reg_no]')\" class='btn-small btn-primary' style='text-decoration:none;padding:5px 15px;'>BAYAR</a></div> "; ?>
					</td>
	...
	public function viewbayar($query)
	{
		$query = $this->db->prepare($query);
		$query->execute();
		$no = 1;
		...
		else
		{
			?>
            <tr>
           		<td colspan="4"><strong>Data Kosong</strong></td>
            </tr>
            <?php
		}		
	}
